...
if ( isset( $_POST[ "submit" ] ) ) {
  ...
  if ( $error == false ) {
	...
	try {
	  ...
	  //If user submitted phone number 
	  if (!empty ($phone)) {
		//Insert database record with phone number
	   ...
	   echo "$confirmation";
		...
		/***********
	  //If user did not submit phone number
	  else {
		//Insert database record without phone number
		$insertSQL = "INSERT INTO `teaven_contacts`(`fname`, `lname`, `email`, `message`) VALUES ( :fname, :lname, :email, :message )";
			
		//Execute query	
		$stmt = $dbConn->prepare($insertSQL);
		$stmt->execute(array(':fname' => $fname, ':lname' => $lname, ':email' => $email, ':message' => $message));  
	  ...
	  //Output error message
	  echo "<p>Sorry there was a problem. Please <a href='$referer'>try again.</a></p>\n";
	  //Log errors
	  log_error($e);
	}
