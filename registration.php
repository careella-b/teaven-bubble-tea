...
<link href="../site/loginForm.css" rel="stylesheet">
  ...
  <div class="container"> <a href="home.php" class="navbar-brand"> 
    ...
    <div id="navbarSupportedContent" class="collapse navbar-collapse">
      <ul class="navbar-nav ml-auto">
        ...
        <!-- This checks if user is logged-in, if so shows log out button  and allows access to account page but if not then shows log in button and restrcits access to account page. -->
        <?php
...
<main>
   ...
   <form id="registrationForm" action="registrationProcess.php" method="post" >
            <div class="form-group first">
              ...
              ?>
              <!--Referer in form as a hidden value so it can be retrieved by registrationProcess.php-->
              <input type="hidden" name="referer" value="<?= $referer?>" />
			  <label for="firstname"></label>
              <input type="text" name="firstname" id="firstname" placeholder="First Name" pattern="[a-zA-Z]+" title="Only alphabets are allowed." size="20" minlenght="1" maxlength="20" accesskey="f" class="form-control" tabindex="1" required />
			  <label for="surname"></label>
              <input type="text" name="surname" id="surname" placeholder="Surame" pattern="[a-zA-Z]+" title="Only alphabets are allowed." size="20" minlenght="1" maxlength="20" accesskey="l" class="form-control" tabindex="2" required />
              <label for="username"></label>
              <input type="text" id="username" id="username" size="30" tabindex="3" placeholder="Username" accesskey="u" name="username" class="form-control" pattern="^[A-Za-z0-9_]{1,15}$" size="20" maxlength="20" required >
              <!--  usernames have a character maximum of 15--> 
            </div>
            <div class="form-group last mb-4">
              <label for="newPassword"></label>
              <input type="password" id="newPassword" size="50" tabindex="2" placeholder="New Password (8 characters minimum)" accesskey="n" name="newPassword"  minlength="8" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" class="form-control" tabindex="4" required>
              <!--Password (UpperCase, LowerCase, Number/SpecialChar and min 8 Chars)--> 
			  <label for="confirmPassword"></label>
              <input type="password" id="confirmPassword" size="50" tabindex="2" placeholder="Confirm Password (8 characters minimum)" accesskey="c" name="confirmPassword" minlength="8" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" class="form-control" tabindex="5" required>
            </div>
            <input type="submit" value="Log In" name="submit" id="rsubmit" class="btn text-white btn-block btn-primary" tabindex="6" class="form-control" accesskey="s">
          </form>
</main>
<!-- End of main -->
