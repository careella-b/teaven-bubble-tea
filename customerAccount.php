      ...
      <div class="grid-row">
        <div class="grid-col-12of12">
          <div class="section-box">
            ...
            <p class="text-center">
			<!-- Connecting to the order database, fetch the orders and display them to the user -->
              <?php
              try {
                ...
                $selectSQL = "SELECT order_id, category_name, flavour_name, topping_name, size_name, order_quantity, order_price
							FROM teaven_orders
							INNER JOIN teaven_category
							ON teaven_category.category_id = teaven_orders.order_cat
							INNER JOIN teaven_flavours
							ON teaven_flavours.flavour_id = teaven_orders.order_flavour
							INNER JOIN teaven_toppings
							ON teaven_toppings.topping_id = teaven_orders.order_topping
							INNER JOIN teaven_size
							ON teaven_size.size_id = teaven_orders.order_size";
                // Execute the query
                $queryResult = $dbConn->query( $selectSQL );
                $rowObj = $queryResult->fetchObject();
				  // Display orders in bullet points 
                echo "<div class='order-list text-center'>
					<ul>
					<li>Bubble Type: {$rowObj->category_name}</li>
					<li>Flavour: {$rowObj->flavour_name}</li>
					<li>Topping: {$rowObj->topping_name}</li>
					<li>Size: {$rowObj->size_name}</li> <li>Quantity: {$rowObj->order_quantity}</li>
					<li>Order ID: {$rowObj->order_id}</li>
					</ul>
					</div>\n";
